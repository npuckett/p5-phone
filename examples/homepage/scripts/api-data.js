window.P5PHONE_PERMISSION_MATRIX = [
  { capability: 'Motion sensors', status: 'window.sensorsEnabled', tap: 'enableGyroTap(message)', button: 'enableGyroButton(text)', canvas: 'enableGyroCanvas(message)', banner: 'enableGyroBanner(message, position)', minimal: 'enableGyroMinimal(message|opts)', custom: 'enableGyroOn(selector)', notes: 'Use for rotationX/Y/Z, accelerationX/Y/Z, deviceMoved(), and deviceShaken().' },
  { capability: 'Microphone', status: 'window.micOpen (live) / window.micEnabled (requested)', tap: 'enableMicTap(message)', button: 'enableMicButton(text)', canvas: 'enableMicCanvas(message)', banner: 'enableMicBanner(message, position)', minimal: 'enableMicMinimal(message|opts)', custom: 'enableMicOn(selector)', notes: 'Requires p5.sound when using p5.AudioIn. Read the level with mic.connect(amplitude); amplitude.getLevel() (after mic.disconnect()).' },
  { capability: 'Sound output', status: 'window.soundEnabled', tap: 'enableSoundTap(message)', button: 'enableSoundButton(text)', canvas: 'enableSoundCanvas(message)', banner: 'enableSoundBanner(message, position)', minimal: 'enableSoundMinimal(message|opts)', custom: 'enableSoundOn(selector)', notes: 'Unlocks browser audio playback after a user gesture.' },
  { capability: 'Speech recognition', status: 'window.speechEnabled', tap: 'enableSpeechTap(message)', button: 'enableSpeechButton(text)', canvas: 'enableSpeechCanvas(message)', banner: 'enableSpeechBanner(message, position)', minimal: 'enableSpeechMinimal(message|opts)', custom: 'enableSpeechOn(selector)', notes: 'Activates audio context. Create your own p5.SpeechRec after permission.' },
  { capability: 'Vibration', status: 'window.vibrationEnabled', tap: 'enableVibrationTap(message)', button: 'enableVibrationButton(text)', canvas: 'enableVibrationCanvas(message)', banner: 'enableVibrationBanner(message, position)', minimal: 'enableVibrationMinimal(message|opts)', custom: 'enableVibrationOn(selector)', notes: 'Android-oriented. Use vibrate(pattern) for haptics.' },
  { capability: 'Torch / flashlight', status: 'window.torchEnabled, window.torchActive', tap: 'enableTorchTap(message)', button: 'enableTorchButton(text)', canvas: 'enableTorchCanvas(message)', banner: 'enableTorchBanner(message, position)', minimal: 'enableTorchMinimal(message|opts)', custom: 'enableTorchOn(selector)', notes: 'Android Chrome-oriented. Starts a rear camera stream and controls the flashlight with torchOn(), torchOff(), and toggleTorch().' },
...
window.P5PHONE_API_SECTIONS = [
  ...
  },
  {
    ...
    items: [
      { name: 'shareSetup', signature: "shareSetup({ host, room, app, shared, me, sendInterval, disconnectWhenHidden })", summary: 'Configures the PartyServer host and room. Seeds shared (room) and me (this client). URL ?shareHost=&room=&app= override host/room/app when present.', tags: ['share', 'setup'] },
