...
- Include `p5@2.2.3`, `p5.js-compatibility@0.2.0`, and `p5-phone@1.10.0` in generated HTML.
...
- Request permissions from a user action using `enableSensorTap`, `enableMicTap`, `enableSoundTap`, `enableSpeechTap`, `enableCameraTap`, `enableNfcTap`, `enableVibrationTap`, `enablePermissionsTap`, or style variants.
- Use one `enablePermissions*` / `enableHardware*` call for sketches that need multiple hardware features from the same gesture, such as `enablePermissionsTap(['sensors', 'mic'])`.
...
- Gate hardware-dependent drawing behind `window.sensorsEnabled`, `window.micEnabled`, `window.speechEnabled`, `window.nfcEnabled`, `window.cameraEnabled`, or `window.vibrationEnabled`.
...
- Any combination: `enablePermissions*` or `enableHardware*` with `sensors`, `mic`, `sound`, `speech`, `vibration`, `nfc`, and `camera`; aliases include `gyro`, `microphone`, `video`, and `haptics`.
